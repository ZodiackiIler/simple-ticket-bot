  ...
  async execute(interaction, client) {
    ...
    if (interaction.customId == "open-ticket") {
      if (client.guilds.cache.get(interaction.guildId).channels.cache.find(c => c.topic == interaction.user.id)) {
        return interaction.reply({
          content: 'у вас уже есть созданный билет!',
          ephemeral: true
        });
      };
      ...
      interaction.guild.channels.create(`ticket-${interaction.user.username}`, {
        ...
        collector.on('end', collected => {
          if (collected.size < 1) {
            c.send(`Не было никакой причины, билет будет закрыт.`).then(() => {
              setTimeout(() => {
                if (c.deletable) {
                  c.delete();
                };
    ...
    if (interaction.customId == "close-ticket") {
      ...
      const verif = await interaction.reply({
        content: 'Вы уверены, что хотите закрыть билет?',
        components: [row]
      });
      ...
      collector.on('collect', i => {
        if (i.customId == 'confirm-close') {
          interaction.editReply({
            content: `Билет был закрыт <@!${interaction.user.id}>`,
            components: []
          });
          ...
          chan.edit({
            ...
            .then(async () => {
              const embed = new client.discord.MessageEmbed()
                .setColor('ff9600')
                .setAuthor('Билет', ' ')
                .setDescription('```Сохранение билета```')
                .setFooter('Билетная система', ' ')
                .setTimestamp();
        ...
        if (i.customId == 'no') {
          interaction.editReply({
            content: 'Закрытие билета отменено!',
            components: []
          });
          collector.stop();
        };
      ...
      collector.on('end', (i) => {
        if (i.size < 1) {
          interaction.editReply({
            content: 'Закрытие билетов отменено!',
            components: []
          });
        };
      });
    ...
    if (interaction.customId == "delete-ticket") {
      ...
      chan.messages.fetch().then(async (messages) => {
        ...
        if (a.length < 1) a = "Это не было написано в билете"
